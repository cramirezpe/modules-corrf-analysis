...
class TestCommon(unittest.TestCase):
    ...
    def test_volume_between_zs(self):
        self.assertAlmostEqual(
            self.theory.get_volume_between_zs(0.6, 0.1).value, 44683901350.33176
        )
        self.assertAlmostEqual(
            self.theory.get_volume_between_zs(0.3).value, 7137310306.571973
        )
    ...
    def test_get_nz_histogram_from_CoLoRe_box(self):
        hist = self.theory.get_nz_histogram_from_CoLoRe_box(
            np.linspace(1.38, 1.41, 10), rsd=False
        )
        values = np.asarray(
            [33.3333333, 55.5555556, 33.3333333, 100.0, 44.4444444, 33.3333333, 0, 0, 0]
        )
        np.testing.assert_almost_equal(hist, values)

        hist = self.theory.get_nz_histogram_from_CoLoRe_box(
            np.linspace(1.38, 1.41, 10), rsd=True
        )
        values = np.asarray(
            [32.1428571, 53.5714286, 75.0, 64.2857143, 64.2857143, 10.7142857, 0, 0, 0]
        )
        np.testing.assert_almost_equal(hist, values)
    ...
    def test_logarithmic_growth_rate(self):
        self.assertAlmostEqual(
            self.theory.logarithmic_growth_rate(0.8, read_file=False),
            0.8315978564605921,
        )
...
class TestComputeModelsCoLoRe(unittest.TestCase):
    def setUp(self):
        ...
        self.theory_dm = ComputeModelsCoLoRe(
            self.sim_path,
            source=2,
            nz_filename=self.nz_filename,
            bias_filename=self.bias_filename,
        ...
        self.theory_mm = ComputeModelsCoLoRe(
            self.sim_path,
            source=2,
            nz_filename=self.nz_filename,
            bias_filename=self.bias_filename,
        ...
        self.theory_nolog = ComputeModelsCoLoRe(
            self.sim_path,
            source=2,
            nz_filename=self.nz_filename,
            bias_filename=self.bias_filename,
        ...
        self.theory_smoothings = ComputeModelsCoLoRe(
            self.sim_path,
            source=2,
            nz_filename=self.nz_filename,
            bias_filename=self.bias_filename,
        ...
        self.theory_with_analysis_smoothing = ComputeModelsCoLoRe(
            self.sim_path,
            source=2,
            nz_filename=self.nz_filename,
            bias_filename=self.bias_filename,
    ...
    def test_k(self):
        np.testing.assert_almost_equal(
            self.theory.k[:5], [0.0001, 0.000102, 0.0001041, 0.0001062, 0.0001083]
        )
    ...
    def test_get_theory_pk_fixed_bias_3(self):
        _, _pk = self.theory_dm.get_theory_pk(
            z=0.3, bias=3, lognormal=False, tracer="dm"
        )
        mean = np.mean(_pk)
        std = np.std(_pk)
    ...
    def test_get_theory_pk_fixed_bias_4(self):
        _, _pk = self.theory_mm.get_theory_pk(
            z=0.3, bias=3, lognormal=False, tracer="mm"
        )
        mean = np.mean(_pk)
        std = np.std(_pk)
    ...
    def test_get_theory_pk_2(self):
        _, pk = self.theory_smoothings.get_theory_pk(
            z=0.3,
            lognormal=False,
            smooth_factor=self.theory_smoothings.smooth_factor_rsd,
        )
        mean = np.mean(pk)
        std = np.std(pk)
    ...
    def test_get_npole_pk_playing_with_smoothings(self):
        ...
        pk = self.theory_smoothings.get_npole_pk(
            0,
            0.3,
            rsd=False,
    ...
    def test_get_npole_pk_7(self):
        pk = self.theory_with_analysis_smoothing.get_npole_pk(
            0, 0.3, rsd=True, bias=0.4
        )
        mean = np.mean(pk)
        std = np.std(pk)
    ...
    def test_get_npole(self):
        ...
        std = np.std(xi)
        np.testing.assert_almost_equal(
            [mean, std], [0.0008141703529694578, 0.001857607536922607]
        )

    def test_mixing_smoothings(self):
        z = 0.4
        pk_l = self.theory_smoothings.get_theory_pk(
            z,
            bias=None,
            lognormal=True,
            smooth_factor=self.theory_smoothings.smooth_factor,
        )[1]
        pk_s = self.theory_smoothings.get_theory_pk(
            z,
            bias=None,
            lognormal=False,
            smooth_factor=self.theory_smoothings.smooth_factor_rsd,
...
class TestLyaBox(unittest.TestCase):
    ...
    def test_get_nz_from_master(self):
        value = self.theory.get_nz_histogram_from_master_file(
            self.master_file, bins=np.linspace(2.2, 2.4, 8), rsd=True
        )
        target = np.array(
            [
                5.0954654,
        ...
        value = self.theory.get_nz_histogram_from_master_file(
            self.master_file, bins=np.linspace(2.2, 2.4, 8), rsd=False
        )
        target = np.array(
            [
                5.2021403,
    ...
    def test_get_zeff(self):
        value = self.theory.get_zeff(
            zmin=2.1, zmax=2.4, method="master_file", master_file=self.master_file
        )
    ...
    def test_combine_z_npoles_pk(self):
        pk = self.theory.combine_z_npoles(
            0,
            [2.3, 2.4, 2.8],
            rsd=False,
            mode="pk",
    ...
    def test_combine_z_npoles_xi(self):
        xi = self.theory.combine_z_npoles(
            0,
            [2.3, 2.4, 2.8],
            rsd=False,
            mode="xi",
