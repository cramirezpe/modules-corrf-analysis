...
class ReadCoLoRe:
    ...
    _default_Ode0 = 0.7
    _default_Ob0 = 0.05
    _default_cosmo = astropy.cosmology.LambdaCDM(
        _default_h * 100, Om0=_default_Om0, Ode0=_default_Ode0, Ob0=_default_Ob0
    )
    ...
    def __init__(
        ...
        if bias_filename is None:  # pragma: no cover
            try:
                self.bias_filename = self.param_cfg[f"srcs{self.source}"][
                    "bias_filename"
                ]
            except Exception:
    ...
    def L_box(self):
        ...
        r_max = (
            cosmo.comoving_distance(float(self.param_cfg["global"]["z_max"]))
            * self.cosmo["h"]
        )
    ...
    def get_zeff(
        self, zmin, zmax, nbins=100, method="Nz_file", master_file=None, rsd=True
    ):
        ...
        if method == "CoLoRe":
            ...
            if master_file is None:  # pragma: no cover
                raise ValueError(
                    "master_file needs to be defined to use master_file method"
                )
            else:
    ...
    def z_bins_from_files(self):
        ...
        return np.sort(
            np.array(
                [
                    float(x.name[18:23])
                    for x in self.box_path.glob(f"out_pk_srcs_pop{self.source-1}*")
                ]
            )
        )
    ...
    def get_a_eq(self):  # pragma: no cover
        ...
        if self.cosmo["omega_L"] != 0:
            if self.cosmo["normalDE"] == 1:
                ...
                aeqL = (self.cosmo["omega_M"] / self.cosmo["omega_L"]) ** (
                    -1 / (3 * self.cosmo["w"])
                )
        ...
        def dum(a):  # pragma: no cover
            if self.cosmo["normalDE"]:
                ...
                dum = np.sqrt(
                    ...
                    / (
                        self.cosmo["omega_M"]
                        + self.cosmo["omega_L"] * a ** (-3 * self.cosmo["w"])
                        + self.cosmo["omega_K"] * a
                    )
                )
            else:
    ...
    def logarithmic_growth_rate(self, z, read_file=True):
        ...
        if read_file:
            ...
            f = (
                0.5
                * (
                    5 * self.cosmo["omega_M"] * a / D
                    - (
                        3 * self.cosmo["omega_M"]
                        + 3 * coeff * self.cosmo["omega_L"] * apow
                        + 2 * self.cosmo["omega_K"] * a
                    )
                )
                / (
                    self.cosmo["omega_M"]
                    + self.cosmo["omega_L"] * apow
                    + self.cosmo["omega_K"] * a
                )
            )
        return f
...
class ComputeModelsCoLoRe(ReadCoLoRe):
    def __init__(
        ...
        zmax=None,
        smooth_factor=1.1,
        smooth_factor_rsd=1.0,
        smooth_factor_cross=2.1,
    ...
    def get_npole_pk(
        ...
        """
        smooth_factor = (
            smooth_factor if smooth_factor is not None else self.smooth_factor
        )
        smooth_factor_rsd = (
            smooth_factor_rsd
            if smooth_factor_rsd is not None
            else self.smooth_factor_rsd
        )
        smooth_factor_cross = (
            smooth_factor_cross
            if smooth_factor_cross is not None
            else self.smooth_factor_cross
        )
    ...
    def combine_z_npoles(
        self, n, zbins, rsd, mode="pk", bias=None, method="Nz_file", master_file=None
    ):
        ...
        z_effs = [
            self.get_zeff(zmin, zmax, method=method, master_file=master_file, rsd=rsd)
        ...
        if mode == "pk":
            pks = np.array(
                [
                    self.get_npole_pk(n=n, z=zi, rsd=rsd, bias=biasi) * Nzi * z_width
                    for zi, Nzi, biasi, z_width in zip(z_effs, Nz2, bias, z_widths)
                ]
            )
            ...
            xis = np.array(
                [
                    self.get_npole(n=n, z=zi, rsd=rsd, bias=biasi) * Nzi * z_width
                    for zi, Nzi, biasi, z_width in zip(z_effs, Nz2, bias, z_widths)
                ]
            )
