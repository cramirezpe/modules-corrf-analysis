...
def main():
    parser = argparse.ArgumentParser(
        description="Copy counts files from one analysis folder to another"
    )
    ...
    parser.add_argument(
        "--out-boxes", type=int, nargs="+", help="Output boxes to copy files on"
    )
    ...
    if args.counts_to_copy_as == None:
        ...
        raise ValueError(
            "counts-to-copy and counts-to-copy-as should have the same size."
        )
    ...
    copy_counts_from_analysis(
        in_analysis=args.in_analysis,
        out_analysis=args.out_analysis,
        counts_to_copy=args.counts_to_copy,
        counts_to_copy_as=args.counts_to_copy_as,
        nside=args.nside,
        in_rsd=in_rsd,
        in_rsd2=in_rsd2,
        in_zmin=args.in_out_zmin[0],
        in_zmax=args.in_out_zmax[0],
        in_rmin=args.in_out_rmin[0],
        in_rmax=args.in_out_rmax[0],
        in_Nbins=args.in_out_Nbins[0],
        out_rsd=out_rsd,
        out_rsd2=out_rsd2,
        out_zmin=args.in_out_zmin[1],
        out_zmax=args.in_out_zmax[1],
        out_rmin=args.in_out_rmin[1],
        out_rmax=args.in_out_rmax[1],
        out_Nbins=args.in_out_Nbins[1],
        in_boxes=args.in_boxes,
        out_boxes=args.out_boxes,
    )


def copy_counts_from_analysis(
    in_analysis,
    out_analysis,
    counts_to_copy,
    counts_to_copy_as,
    ...
    full_in_path = FileFuncs.get_full_path(
        in_analysis,
        in_rsd,
        in_rmin,
        in_rmax,
    ...
    full_out_path = FileFuncs.get_full_path(
        out_analysis,
        out_rsd,
        out_rmin,
        out_rmax,
    ...
    for in_box, out_box in zip(in_boxes, out_boxes):
        for pixel in in_box.iterdir():
            (full_out_path / out_box.name / pixel.name).mkdir(
