...
def getArgs():
    ...
    parser.add_argument('--from-nz-file', type=Path, required=False, help='Compute randoms from given dndz file')
...
def main(args=None):
    ...
    if args.from_nz_file != None:
        ...
        rand.generate_random_redshifts_from_file(args.from_nz_file, zmin=args.zmin, zmax=args.zmax)

    if args.from_cat != None:
        rand = FieldData(args.from_cat, 'Randoms', file_type='zcat')    
        rand.prepare_dat(args.zmin, args.zmax, args.downsampling, args.pixel_mask, args.nside)
