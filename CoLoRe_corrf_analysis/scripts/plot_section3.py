...
def getArgs():
    ...
    parser.add_argument(
        "--output-file",
        type=Path,
        default=Path("multipoles_3d_clustering_norsd.pdf"),
        help="Output for the plot",
    )
    parser.add_argument(
        "--log-level",
        default="WARNING",
...
def main(args=None):
    ...
    basedir = Path(
        "/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/hanyu_david_box"
    )
    ...
    for analysis, analysis_rsd in zip(analyses_norsd, analyses_rsd):
        ...
        analysis_rsd.fitter = Fitter(
            boxes=analysis_rsd.sub_boxes,
            z=analysis_rsd.zeff,
            theory=theory,
            poles=[],
            rsd=analysis_rsd.rsd,
            rmin={0: 200, 2: 200},
            rmax={0: 200, 2: 200},
            smooth_factor0=1.1,
            smooth_factor_rsd0=1,
    ...
    for analysis, ax in zip(analyses_norsd + analyses_rsd, axs):
        ...
        for pole in poles:
            ...
            Plots.plot_best_fit(
                ...
                pole=pole,
                ax=ax,
                plot_args=dict(c=c, lw=1, label=label),
                no_labels=True,
            )
            # ----------
