...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Sorting by then by.. options: nside, rsd, rmin, rmax, zmin, zmax, N",
    )
    parser.add_argument(
        "--reverse",
        type=str,
        nargs="+",
        default=None,
        help="Reverse previous sorting. All of them if True, each one if list of bools",
    )
    parser.add_argument(
        "--show-incompleted",
        action="store_true",
        help="Print path to sub-boxes that are not completed.",
    )
    ...
    table = FileFuncs.get_available_runs(
        args.path, args.sort, reverse, args.show_incompleted
    )
    print(table)
