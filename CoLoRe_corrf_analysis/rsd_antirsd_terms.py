...
class Term:
    ...
    def get_boxes(
        ...
        if force_path != None:
            ...
            boxes_path = (
                self.basedir
                / f"s{self.source1}{rsd1_rev_str}_s{self.source2}{rsd2_rev_str}"
            )
    ...
    def model(self, pole):
        _xi = self.theory.get_npole(
            ...
            rsd2=self.rsd2,
            bias2=self.bias2,
            reverse_rsd=self.rsd1_rev,
            reverse_rsd2=self.rsd2_rev,
        )
        return _xi
...
class MixedTerm(Term):
    ...
    @classmethod
    def mix_terms_from_array(
        cls, terms, positive_indices, negative_indices, label=None
    ):
        positive = []
        negative = []
...
class AndreuTerms:
    ...
    def create_term(
        ...
        rsd2_rev=False,
        basedir=Path(
            f"/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/multibias2_0.5_4/multibias2_0.5_4/"
        ),
    ):
        term = Term(
            basedir,
    ...
    @classmethod
    def create_Andreu_terms(
        cls,
        basedir=Path(
            f"/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/multibias2_0.5_4/"
        ),
        get_boxes_kwargs=dict(
            rmin=0.1, rmax=200, N_bins=41, zmin=0.5, zmax=0.7, nside=2, force_path=None
        ),
        get_theory_kwargs=dict(
            box_path=Path("/global/cscratch1/sd/damonge/CoLoRe_sims/sim1000"),
            source=2,
            nz_filename=Path(
                "/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/hanyu_david_box/input_files/NzBlue.txt"
            ),
            pk_filename=Path(
                "/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/hanyu_david_box/input_files/Pk_CAMB_test.dat"
            ),
            param_cfg_filename=Path(
                "/global/cscratch1/sd/damonge/CoLoRe_sims/sim1000/out_params.cfg"
            ),
            bias_filename=Path(
                "/global/project/projectdirs/desi/users/cramirez/CoLoRe_analysis/NbodyKit/hanyu_david_box/input_files/BzBlue.txt"
            ),
        ...
        for _key, _item in terms_pars.items():
            ...
            _item["bias2"] = (
                biases_dict[_item["source2"]] if _item["source2"] != None else None
            )

            terms[_key] = cls.create_term(
                get_boxes_kwargs,
                get_theory_kwargs,
                label=_key,
                **_item,
        ...
        terms[11] = MixedTerm.mix_terms_from_array(
            terms, positive_indices=[3], negative_indices=[1, 2], label=11
        )
        terms[12] = MixedTerm.mix_terms_from_array(
            terms, positive_indices=[5], negative_indices=[2, 4], label=12
        )
        terms[13] = MixedTerm.mix_terms_from_array(
            terms,
            positive_indices=[6, 2, 2, 4, 1],
            negative_indices=[5, 5, 3, 3],
            label=13,
        )
...
class TermsPlots:
    ...
    def plot_term(ax, term, poles=[0, 2], c=["C0"], **kwargs):
        ...
        for pole, ci in zip(poles, c):
            ...
            ax.errorbar(
                term.rdata,
                term.rdata ** 2 * _dataxi,
                term.rdata ** 2 * _dataxierr,
                c=ci,
                fmt=".",
                **args,
            )
